...
async function countItems(id) {
  const { rows } = await pool.query(
    `SELECT COUNT (id) FROM origami_courses WHERE id_category = ${id}`
  );
...
async function insertCourse(courseToAdd) {
  ...
  await pool.query(
    `INSERT INTO origami_courses(name, type, id_category, id_level) VALUES ('${courseToAdd.name}','${courseToAdd.type}',${idCategory},${idLevel})`
  );
}
...
async function updateCourse(courseToUpdate) {
  ...
  await pool.query(
    `UPDATE origami_courses SET name = '${courseToUpdate.name}', type = '${courseToUpdate.type}', id_category = ${idCategory}, id_level = ${idLevel}  WHERE id = ${idToUpdate} `
  );
}
...
async function insertCategory(categoryToAdd) {
  switch (categoryToAdd !== '') {
    case true:
      await pool.query(
        `INSERT INTO categories(category) VALUES ('${categoryToAdd}')`
      );
      break;
    case false:
      break;
...
async function updateCategory(id, nameUpdated) {
  ...
  await pool.query(
    `UPDATE categories SET category = '${nameUpdated}' WHERE id = ${idToUpdate}`
  );
}
